...
test('Custom Resource depends on requestor function, role and default policy for correct deletion order', () => {
  const app = new cdk.App()
  const stack = new cdk.Stack(app, 'TestStack', {})
  ...
  template.hasResource(
    'Custom::DnsValidatedCertificate',
    Match.objectLike({
      DependsOn: Match.arrayWith([
        Match.stringLikeRegexp('.*RequestorFunction.*'),
        Match.stringLikeRegexp('.*DefaultPolicy.*'),
